...
def load_css():
    st.markdown("""
        <style>
            ...
            .header-title { font-size: 1.4rem; font-weight: 700; letter-spacing: 1px; color: white; }
            .header-subtitle { font-size: 0.8rem; color: var(--brand-orange); text-transform: uppercase; margin-left: 10px; }
            ...
            /* Success/Warning */
            .stSuccess { background-color: rgba(140, 198, 63, 0.1); border: 1px solid var(--brand-green); color: var(--brand-green); }
            .stWarning { background-color: rgba(245, 128, 37, 0.1); border: 1px solid var(--brand-orange); color: var(--brand-orange); }
...
# --- HELPER: Handles "Add New" Logic ---
def get_selection(label, options, key_prefix):
    """Creates a selectbox that allows adding a new option."""
    selected = st.selectbox(label, options + ["➕ Add New..."], key=f"{key_prefix}_select")
    
    if selected == "➕ Add New...":
        new_val = st.text_input(f"Enter new {label.lower()}", key=f"{key_prefix}_custom")
        return new_val if new_val else None # Return None if they haven't typed yet
    return selected
    ...
    with col_log:
        ...
        with st.container():
            ...
            with st.form("logger"):
                ...
                customer_verbatim = st.text_input("What did the customer say/see?", placeholder="e.g. 'The button feels mushy and nothing happens'")

                st.caption("2. THE CONTEXT")
                c1, c2 = st.columns(2)
                with c1:
                    # UPDATED: Reusable function handles the "Add New" logic
                    lifecycle = get_selection("Lifecycle Phase", ["Spring Startup", "Daily Operation", "Winterizing", "New Install"], "life")
                with c2:
                    age = get_selection("Park Age", ["Unknown", "< 1 Year", "1-5 Years", "5-10 Years", "10+ Years"], "age")

                c3, c4 = st.columns(2)
                with c3:
                    tech_category = get_selection("Component", ["Activator (Button)", "Solenoid Valve", "Controller", "Pump", "Structure/Fiberglass"], "cat")
                with c4:
                    root_cause_type = get_selection("Root Cause", ["Wear & Tear", "Debris / Blockage", "Installation Error", "Manufacturing Defect", "Environmental (Freeze/Lightning)", "User Error"], "root")

                st.caption("3. THE TECHNICAL FIX")
                specific_diagnosis = st.text_input("Specific Diagnosis", placeholder="e.g. Polara Switch failed (Internal spring collapsed)")
                fix_action = st.text_area("Resolution / Action Taken", placeholder="e.g. Replaced switch with new Black Polara unit. Verified click.")
                
                submitted = st.form_submit_button("💾 Save to Hive Mind")
                
                if submitted:
                    # Validation: Ensure they didn't leave a "Add New" text box empty
                    if not all([lifecycle, age, tech_category, root_cause_type]):
                         st.error("⚠️ Please fill in all 'Add New' fields.")
                    elif customer_verbatim and fix_action:
                        new_case = {
